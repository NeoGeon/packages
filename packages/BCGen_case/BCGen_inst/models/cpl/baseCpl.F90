...
use base_io
    ...
    type(mct_gsMap)          :: gsMap_atmx
    type(mct_gsMap), pointer ::gsMap_ocnocn
    type(mct_gsMap)          :: gsMap_ocnx

    ! declare AttrVect of each Model(c2x_cx, c2x_cc, x2c_cx, x2c_cc)
    type(mct_aVect), pointer :: atm2x_atmatm
    type(mct_aVect), pointer :: atm2x_atmx
    type(mct_aVect), pointer :: x2atm_atmatm
    type(mct_aVect), pointer :: x2atm_atmx
    type(mct_aVect), pointer :: ocn2x_ocnocn
    type(mct_aVect), pointer :: ocn2x_ocnx
    type(mct_aVect), pointer :: x2ocn_ocnocn
    type(mct_aVect), pointer :: x2ocn_ocnx

    ! declare Temp Merge AttrVect of each Model(m2x_nx) 
    type(mct_aVect) ::atm2x_ocnx
    type(mct_aVect) ::ocn2x_atmx

    type(mct_gGrid),  pointer :: domain_atm
    type(mct_gGrid),  pointer :: domain_ocn
...
subroutine cpl_init()
    ...
    if(metaData%iamin_cpl)then
        call avect_init_ext(metaData, atm2x_atmx, metaData%cplid, &
                           ...
                           metaData%modelocn2cpl_id)
        call mapper_spmat_init_rc(metaData%mapper_Smatatm2ocn, gsMap_atmx, gsMap_ocnx, metaData%mpi_cpl,metaData%datarc, &
                           'mapper_Smatatm2ocn', 'X')   
        call avect_init_ext(metaData, ocn2x_ocnx, metaData%cplid, &
                           ocn2x_atmx, metaData%cplid, gsMap_atmx, &
                           metaData%modelatm2cpl_id)
        call mapper_spmat_init_rc(metaData%mapper_Smatocn2atm, gsMap_ocnx, gsMap_atmx, metaData%mpi_cpl,metaData%datarc, &
                           'mapper_Smatocn2atm', 'X')   
    end if
...
subroutine cpl_run()
    ...
    do while(.not. stop_alarm)
        ...
        atm_run = time_alarmIsOn(SyncClock%ECP(clock_drv)%EClock, alarm_atmrun_name)
        ocn_run = time_alarmIsOn(SyncClock%ECP(clock_drv)%EClock, alarm_ocnrun_name)
        if(time_alarmIsOn(EClock_drv, alarm_datestop_name))then
            ...
            if(atm_run)then
        if(metaData%iamin_modelatm2cpl)then
            call mapper_comp_map(metaData%Mapper_Cx2atm, x2atm_atmx, x2atm_atmatm, msgtag=103, ierr=ierr)
        end if
    end if
            ...
            if(ocn_run)then
        if(metaData%iamin_modelocn2cpl)then
            call mapper_comp_map(metaData%Mapper_Cx2ocn, x2ocn_ocnx, x2ocn_ocnocn, msgtag=103, ierr=ierr)
        end if
    end if
        ...
        if(metaData%iamin_modelocn)then
            ...
            if(atm_run)then
        if(metaData%iamin_modelatm2cpl)then
            call mapper_comp_map(metaData%Mapper_Catm2x, atm2x_atmatm, atm2x_atmx, msgtag=103, ierr=ierr)
        end if
    end if
            ...
            if(ocn_run)then
        if(metaData%iamin_modelocn2cpl)then
            call mapper_comp_map(metaData%Mapper_Cocn2x, ocn2x_ocnocn, ocn2x_ocnx, msgtag=103, ierr=ierr)
        end if
    end if
            ...
            if(atm_run)then
        if(metaData%iamin_modelatm2cpl)then
            call mapper_comp_map(metaData%mapper_Smatocn2atm, ocn2x_ocnx, ocn2x_atmx, msgtag=103, field="So_t:So_s:So_u", ierr=ierr)
        end if
    end if
            ...
            if(ocn_run)then
        if(metaData%iamin_modelocn2cpl)then
            call mapper_comp_map(metaData%mapper_Smatatm2ocn, atm2x_atmx, atm2x_ocnx, msgtag=103, field="Sa_z:Sa_u:Sa_v:Sa_tbot:Sa_ptem", ierr=ierr)
        end if
    end if
